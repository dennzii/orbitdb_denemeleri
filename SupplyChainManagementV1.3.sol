...
contract SupplyChainManagement is
    ReentrancyGuard // Starts the definition of the main contract.
{
    ...
    function setQRScanned(bytes32 cargoID,address courierAddr) 
    public 
    onlyOwner
    isCourier(courierAddr)
    isAccountRegistered(courierAddr)
    {
    ...
    function createCargo(address receiver)
        external
    ...
    function receiveCargo(bytes32 id)
        external
        isAccountRegistered(msg.sender)
        isReceiver(id)
        nonReentrant
    ...
    function registerAddress(address addr, AccountTypes accountType)
        external
        onlyOwner
        nonReentrant
    {
    ...
    function getRegisteredAccountDetails(address _addr)
        external
        view
    ...
    function getIDtoCargo(bytes32 id) public view isAccountRegistered(msg.sender) returns( Cargo memory)
    {
        return _IDtoCargo[id];
    }
   
    function getRegisteredAccounts(address addr ) public view  isAccountRegistered(msg.sender) returns(AccountTypes )
    {
        return _registeredAccounts[addr];
    }

    function gettAccToCargoIDs(address addr) public view  isAccountRegistered(msg.sender) returns(bytes32[] memory)
    {
